...
impl Shell {
    ...
    pub fn find_or_create_workspace_id(&mut self, workspace_name: &str) -> WorkspaceId {
        if let Some(&workspace_id) = self.workspace_names.get(workspace_name) {
            ...
            self.workspace_names.insert(workspace_name.to_string(), workspace_id);
    ...
    pub fn switch_workspace_on_virtual(&mut self, virtual_id: VirtualOutputId, workspace_name: &str) {
                    ...
                    if let Some(fallback_id) = fallback_workspace {
                        if let Some(fallback_ws) = self.workspaces.get(&fallback_id) {
                            tracing::debug!("Mapping {} windows for fallback workspace on {:?}", 
                                fallback_ws.windows.len(), current_owner);
    ...
    pub fn add_output(&mut self, output: &Output) {
        ...
        let vouts = self.virtual_output_manager.virtual_outputs_for_physical(output);
        tracing::debug!("Found {} existing virtual outputs for physical output {}", vouts.len(), output.name());
    ...
    fn find_fallback_workspace_for_virtual_output(&mut self, virtual_output_id: VirtualOutputId, exclude_workspace: WorkspaceId) -> Option<WorkspaceId> {
        ...
        for (workspace_id, _) in &self.workspaces {
            ...
            if !is_visible {
                tracing::debug!("Found fallback workspace {:?} for virtual output {:?}", workspace_id, virtual_output_id);
        ...
        tracing::debug!("Created new fallback workspace {:?} ('{}') for virtual output {:?}", 
            workspace_id, next_number, virtual_output_id);
        
        Some(workspace_id)
    }

    /// Add a window to a specific virtual output
    pub fn add_window_to_virtual_output(&mut self, window: Window, virtual_output_id: VirtualOutputId) {
        ...
        tracing::debug!("Adding window to virtual output {:?}, workspace: {:?}", virtual_output_id, workspace_id);
        ...
        let outputs_to_arrange: Vec<_> = if let Some(vout) = self.virtual_output_manager.get(virtual_output_id) {
            vout.regions.iter().map(|region| region.physical_output.clone()).collect()
        } else {
            Vec::new()
        };
    ...
    pub fn add_window(&mut self, window: Window, output: &Output) {
        ...
        let vout = self.virtual_output_manager.virtual_outputs_for_physical(output)
            .first()
        ...
        tracing::debug!("Adding window to virtual output {:?}, workspace: {:?}", vout, workspace_id);
    ...
    pub fn move_window_to_workspace(&mut self, window: Window, workspace_name: String, output: &Output) {
        ...
        let should_map = self.virtual_output_manager.virtual_outputs_for_physical(output)
            .iter()
            .any(|vout| vout.active_workspace() == Some(workspace_id));
    ...
    pub fn get_fullscreen(&self, output: &Output) -> Option<&Window> {
        ...
        for virtual_output in self.virtual_output_manager.virtual_outputs_for_physical(output) {
    ...
    pub fn toggle_fullscreen(&mut self, output: &Output) {
        if let Some(focused_window) = self.focused_window.clone() {
            ...
            let vout_id = self.workspaces.values()
...
//     }
    ...
    pub fn focus_next(&mut self, _output: &Output) {
        ...
        let workspace_name = self.focused_window.as_ref()
            ...
            if workspace.windows.contains(focused_window) {
                ...
                for virtual_output in self.virtual_output_manager.all() {
                    if virtual_output.active_workspace() == Some(*workspace_id) {
                        let workspace_name = self.get_workspace_name(*workspace_id)
    ...
    pub fn virtual_output_at_position(&self, output: &Output, position: Point<f64, Logical>) -> Option<virtual_output::VirtualOutputId> {
    ...
    pub fn workspace_at_position_mut(&mut self, output: &Output, position: Point<f64, Logical>) -> Option<&mut Workspace> {
    ...
    where F: FnMut(&mut Workspace) {
        let workspace_ids: Vec<WorkspaceId> = self.virtual_output_manager.virtual_outputs_for_physical(output)
            .iter()
            .filter_map(|vout| vout.active_workspace())
            .collect();
    ...
    pub fn switch_to_workspace(&mut self, output: &Output, name: String) {
        tracing::info!("Switching to workspace {} on output {} (via virtual output)", name, output.name());
        
        // Find first virtual output on this physical output
        let binding = self.virtual_output_manager.virtual_outputs_for_physical(output);
        let vout_id = binding
            .first()
            .map(|vout| vout.id);
        ...
        if let Some(vout_id) = vout_id {
            ...
            tracing::warn!("No virtual output found for physical output {}", output.name());
        }
    }
    ...
    pub fn arrange_windows_on_output(&mut self, output: &Output) {
        ...
        let virtual_output_info: Vec<_> = self.virtual_output_manager
        ...
        if let Some(workspace_name) = found_workspace_name {
            for virtual_output in self.virtual_output_manager.all() {
                if virtual_output.active_workspace.as_ref() == Some(&workspace_name) {
                    return virtual_output.regions.iter()
                        .map(|r| r.physical_output.clone())
                        .collect();
                }
            }
